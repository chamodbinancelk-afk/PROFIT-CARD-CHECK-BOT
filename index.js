...
                        `🔥 <b>සිංහල:</b> ${description_si}\n\n` + // This variable now holds the direct translation
                        
...
                ]
            };
            await sendRawTelegramMessage(chatId, replyText, null, startReplyMarkup, messageId);
            break;

        case '/fundamental':
            const lastFundamentalMessage = await readKV(env, LAST_FULL_MESSAGE_KEY);
            const lastImageUrl = await readKV(env, LAST_IMAGE_URL_KEY);
            
            if (lastFundamentalMessage) {
                // Send the message, using sendPhoto if imgUrl is available
                await sendRawTelegramMessage(chatId, lastFundamentalMessage, lastImageUrl, null, messageId);
            } else {
                await sendRawTelegramMessage(chatId, "⚠️ <b>Fundamental News Update එකක් තවම හමු වී නැත.</b>\n\nඑය ලැබුණු වහාම පළ කරනු ඇත.", null, null, messageId);
            }
            break;
...
            } else {
                await sendRawTelegramMessage(chatId, "⚠️ <b>Economic Calendar Update එකක් තවම හමු වී නැත.</b>\n\nඑය ලැබුණු වහාම පළ කරනු ඇත.", null, null, messageId);
            }
            break;

        default:
            // Ignore non-command messages or unknown commands
            break;
    }
    // Ensure the function returns to complete the webhook
    return new Response("OK", { status: 200 });
}


// =================================================================
// --- WORKER EXPORT HANDLER (ප්‍රධාන හසුරුවන්නා) ---
// =================================================================

/**
 * The main export handler for the Cloudflare Worker, handling both scheduled events (cron) and HTTP requests (webhooks).
 */
export default {
    /**
     * Handles scheduled events (like cron jobs) to run the scraping logic.
     * @param {ScheduledController} controller - The scheduled event controller.
     * @param {Env} env - The environment variables (including KV binding).
     */
    async scheduled(controller, env) {
        console.log(`[SCHEDULED] Running news fetch at ${new Date().toISOString()}`);
        // Run both tasks concurrently to maximize efficiency
        // Ensure your cron schedule is set appropriately for your desired update frequency.
        await Promise.all([
            fetchForexNews(env),
    ...
    /**
     * Handles incoming HTTP requests (Telegram Webhooks).
    ...
    async fetch(request, env, ctx) {
        if (request.method !== 'POST') {
            return new Response("Method Not Allowed. This Worker is designed to run via scheduled triggers and Telegram webhooks (POST).", { status: 405 });
        }

        const url = new URL(request.url);
        // Implement secret path check if deploying to a publicly accessible domain

        try {
            const update = await request.json();
            // Delegate to the Telegram handler
            await handleTelegramUpdate(update, env);
            
            // Telegram expects a quick 200 OK response, even if processing had an internal issue
            return new Response("OK", { status: 200 });
        } catch (error) {
            console.error("Webhook processing error:", error.stack);
            // Always return 200 to Telegram to prevent persistent retries
            return new Response("Internal Error Processed", { status: 200 });
        }
    }
};
